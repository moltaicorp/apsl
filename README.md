...
| Binary | Purpose |
|---|---|
| `apslc` | Parse, canonicalize, hash, link, type-check, and compile artifacts. |
...
`parse` and `canon` print the canonical AST. `hash` prints its SHA-256 digest. `check` links and type-checks. `compile` emits the checked canonical graph/type artifact.
