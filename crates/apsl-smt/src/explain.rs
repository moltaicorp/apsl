...
pub fn explain(node: &Node, clause_id: usize, model: &Model) -> String {
    ...
    for (i, (k, v)) in model.bindings.iter().enumerate() {
        if i > 0 { bindings.push_str(", "); }
        bindings.push_str(&format!("{} = {}", k, v));
    }
    if bindings.is_empty() { bindings = "<no model bindings reported>".into(); }
    format!(
        "In clause {} of node {}: the post-condition fails when {}.",
        clause_id, node.name.as_str(), bindings
    )
}
...
mod tests {
    ...
    fn explain_includes_node_name() {
        let n = Node {
            ...
            sig: TypeSig { params: vec![], ret: Type::Base(Ident::new("Int")) },
            pre: vec![], post: vec![],
        ...
        let m = Model { bindings: vec![("x".into(), "42".into())] };
        let s = explain(&n, 0, &m);
