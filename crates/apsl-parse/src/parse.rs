...
use apsl_core::ast::{
    BinOp, Bind, CxExpr, CxSpec, Decl, DeployClauses, Duration, Expr, FlowStep, Graph, Ident, Lit, Node, Param,
    AuthLevel, AuditReq, ScopeConstraint, Program, Quant, RuntimeClass, Sla, Span, StateDecl, Type as AstType, TypeAlias, TypeSig, UnOp, Via,
};
...
impl std::fmt::Display for ParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "parse error at line {} col {}: {}", self.span.line, self.span.col, self.msg)
    }
}
...
impl Parser {
    fn new(toks: Vec<Tok>) -> Self { Self { toks, pos: 0, allow_dot_postfix: true } }

    fn peek(&self) -> &TokKind { &self.toks[self.pos].kind }
    fn peek_tok(&self) -> &Tok { &self.toks[self.pos] }
    fn span(&self) -> Span { self.toks[self.pos].span.clone() }
    ...
    fn skip_newlines(&mut self) {
        while matches!(self.peek(), TokKind::Newline) { self.bump(); }
    }
    ...
    fn parse_decl(&mut self) -> Result<Decl, ParseError> {
        ...
        match self.peek().clone() {
            TokKind::Ident(s) if s == "type" => {
                ...
                self.skip_newlines();
                Ok(Decl::Type(TypeAlias { name, rhs, supertypes, span: start }))
            }
    ...
    fn parse_type_name(&mut self) -> Result<Ident, ParseError> {
        match self.peek().clone() {
            TokKind::TypeName(s) => { self.bump(); Ok(Ident::new(s)) }
    ...
    fn parse_ident(&mut self) -> Result<Ident, ParseError> {
        match self.peek().clone() {
            TokKind::Ident(s) => { self.bump(); Ok(Ident::new(s)) }
            TokKind::TypeName(s) => { self.bump(); Ok(Ident::new(s)) }
    ...
    fn parse_type_atom(&mut self) -> Result<AstType, ParseError> {
        match self.peek().clone() {
            ...
            TokKind::LParen => {
                ...
                Ok(if ts.len() == 1 { ts.remove(0) } else { AstType::Tuple(ts) })
            }
            TokKind::LBrace => {
                self.bump();
    ...
    fn parse_typesig(&mut self) -> Result<TypeSig, ParseError> {
        ...
        let params = match t_in {
            AstType::Tuple(ts) => ts.into_iter()
                .enumerate()
                .map(|(i, t)| Param { name: Ident::new(if i == 0 { "in".to_string() } else { format!("in{}", i) }), ty: t })
                .collect(),
            other => vec![Param { name: Ident::new("in"), ty: other }],
        };
            ...
            loop {
                ...
                if let TokKind::Ident(n) = self.peek().clone() {
                    ...
                    if matches!(self.peek(), TokKind::Colon) {
                        ...
                        params.push(Param { name: Ident::new(n), ty });
                        if matches!(self.peek(), TokKind::Comma) { self.bump(); continue; }
                        else { break; }
                    } else {
                        self.pos = save;
                        let _ = nspan;
                ...
                let i = params.len();
                params.push(Param {
                    name: Ident::new(if i == 0 { "in".to_string() } else { format!("in{}", i) }),
                    ty,
                });
                if matches!(self.peek(), TokKind::Comma) { self.bump(); continue; }
                else { break; }
    ...
    fn parse_node(&mut self, start: Span) -> Result<Node, ParseError> {
        ...
        if matches!(self.peek(), TokKind::Indent) {
            ...
            loop {
                ...
                match self.peek().clone() {
                    TokKind::Ident(k) if k == "pre"  => { self.bump(); pre.extend(self.parse_pred_list()?); }
                    TokKind::Ident(k) if k == "post" => { self.bump(); post.extend(self.parse_pred_list()?); }
                    TokKind::Ident(k) if k == "cx"   => { self.bump(); cx = Some(self.parse_cx_spec()?); }
                    TokKind::Ident(k) if k == "sla"  => { self.bump(); sla = Some(self.parse_sla_spec()?); }
                    TokKind::Ident(k) if k == "via"  => { self.bump(); via = Some(self.parse_via_spec()?); }
                    TokKind::Ident(k) if k == "emits"      => { self.bump(); dep.emits = Some(self.parse_ident()?); }
                    TokKind::Ident(k) if k == "stage"      => { self.bump(); dep.stage = Some(self.parse_dashed_ident()?); }
                    TokKind::Ident(k) if k == "image"      => { self.bump(); dep.image = Some(self.parse_state_key()?); }
                    TokKind::Ident(k) if k == "needs"      => { self.bump(); dep.needs = self.parse_needs_list()?; }
                    TokKind::Ident(k) if k == "binds"      => { self.bump(); let b = self.parse_bind()?; dep.binds.push(b); }
                    TokKind::Ident(k) if k == "gate"       => { self.bump(); dep.gate = Some(self.parse_expr()?); }
                    TokKind::Ident(k) if k == "proof"      => { self.bump(); dep.proof = Some(self.parse_ident()?); }
                    TokKind::Ident(k) if k == "delegation" => { self.bump(); dep.delegation = Some(self.parse_key_path()?); }
                    TokKind::Ident(k) if k == "auth" => {
                        self.bump();
                        auth = match self.peek().clone() {
                            TokKind::Ident(a) if a == "none" => { self.bump(); AuthLevel::None }
                            TokKind::Ident(a) if a == "bearer" => { self.bump(); AuthLevel::Bearer }
                            TokKind::Ident(a) if a == "session" => { self.bump(); AuthLevel::Session }
                            TokKind::Ident(a) if a == "passkey" => { self.bump(); AuthLevel::Passkey }
                            other => return Err(self.err(format!("expected auth level (none|bearer|session|passkey), got {:?}", other))),
                        };
                    }
                    TokKind::Ident(k) if k == "scope" => {
                        self.bump();
                        scope_constraint = match self.peek().clone() {
                            TokKind::Ident(s) if s == "any" || s == "*" => { self.bump(); ScopeConstraint::Any }
                            TokKind::Ident(s) if s == "narrowing" => { self.bump(); ScopeConstraint::Narrowing }
                            TokKind::Ident(s) if s == "admitted" => { self.bump(); ScopeConstraint::Admitted }
                            other => return Err(self.err(format!("expected scope (any|narrowing|admitted), got {:?}", other))),
                        };
                    }
                    TokKind::Ident(k) if k == "audit" => {
                        self.bump();
                        audit_req = match self.peek().clone() {
                            TokKind::Ident(a) if a == "none" => { self.bump(); AuditReq::None }
                            TokKind::Ident(a) if a == "before" => { self.bump(); AuditReq::Before }
                            TokKind::Ident(a) if a == "after" => { self.bump(); AuditReq::After }
                            TokKind::Ident(a) if a == "both" => { self.bump(); AuditReq::Both }
                            other => return Err(self.err(format!("expected audit (none|before|after|both), got {:?}", other))),
                        };
                    }
                    ...
                    other => return Err(self.err(format!("expected clause keyword, got {:?}", other))),
                }
                self.skip_newlines();
            }
            if matches!(self.peek(), TokKind::Dedent) { self.bump(); }
        }
        let cx = cx.unwrap_or(CxSpec { bigo: CxExpr::Const, class: RuntimeClass::Idem });
        let deploy = if dep.is_empty() { None } else { Some(dep) };
        Ok(Node { name, sig, pre, post, cx, sla, via, auth, scope_constraint, audit_req, state: state_decls, deploy, span: start })
    }

    fn parse_dashed_ident(&mut self) -> Result<Ident, ParseError> {
        let mut s = match self.peek().clone() {
            TokKind::Ident(x) => { self.bump(); x }
            TokKind::TypeName(x) => { self.bump(); x }
            other => return Err(self.err(format!("expected a name, got {:?}", other))),
        };
        while matches!(self.peek(), TokKind::Minus) {
            self.bump();
            match self.peek().clone() {
                TokKind::Ident(x) => { self.bump(); s.push('-'); s.push_str(&x); }
                TokKind::TypeName(x) => { self.bump(); s.push('-'); s.push_str(&x); }
                other => return Err(self.err(format!("expected a name after `-`, got {:?}", other))),
            }
        }
        Ok(Ident::new(s))
    }
    ...
    fn parse_state_key(&mut self) -> Result<Ident, ParseError> {
        ...
        match self.peek().clone() {
            TokKind::Ident(s) if s == "state" => { self.bump(); }
    ...
    fn parse_needs_list(&mut self) -> Result<Vec<Ident>, ParseError> {
        if matches!(self.peek(), TokKind::BracketPair) { self.bump(); return Ok(Vec::new()); }
        self.expect(&TokKind::LBracket, "`[`")?;
        let mut out = Vec::new();
        if !matches!(self.peek(), TokKind::RBracket) {
            out.push(self.parse_ident()?);
            while matches!(self.peek(), TokKind::Comma) { self.bump(); out.push(self.parse_ident()?); }
        }
    ...
    fn parse_state_decl(&mut self) -> Result<StateDecl, ParseError> {
        ...
        Ok(StateDecl { key, ty, default, span: sp })
    }

    fn parse_lit(&mut self) -> Result<Lit, ParseError> {
        match self.peek().clone() {
            TokKind::IntLit(n) => { self.bump(); Ok(Lit::Int(n)) }
            TokKind::RatLit(p, q) => { self.bump(); Ok(Lit::Rat(p, q)) }
            TokKind::BoolLit(b) => { self.bump(); Ok(Lit::Bool(b)) }
            TokKind::StrLit(s) => { self.bump(); Ok(Lit::Str(s)) }
            other => Err(self.err(format!("expected literal value for state default, got {:?}", other))),
        }
    }
    ...
    fn parse_graph(&mut self, start: Span) -> Result<Graph, ParseError> {
        ...
        if matches!(self.peek(), TokKind::Indent) {
            ...
            loop {
                ...
                match self.peek().clone() {
                    TokKind::Ident(k) if k == "post" => { self.bump(); post.extend(self.parse_pred_list()?); }
                    TokKind::Ident(k) if k == "flow" => { self.bump(); flow.push(self.parse_flow_chain()?); }
                    other => return Err(self.err(format!("expected post or flow, got {:?}", other))),
                }
                self.skip_newlines();
            }
            if matches!(self.peek(), TokKind::Dedent) { self.bump(); }
        }
        Ok(Graph { name, sig, post, flow, state: Vec::new(), span: start })
    }
    ...
    fn parse_cx_spec(&mut self) -> Result<CxSpec, ParseError> {
        match self.peek().clone() {
            TokKind::TypeName(s) if s == "O" => { self.bump(); }
            TokKind::Ident(s) if s == "O" => { self.bump(); }
    ...
    fn parse_runtime_class(&mut self) -> RuntimeClass {
        ...
        if !matches!(first.as_str(), "idem" | "anti") { return RuntimeClass::Idem; }
        self.bump();
        ...
        loop {
            match self.peek().clone() {
                TokKind::Plus => {
                    ...
                    term = match term {
                        CxExpr::Sum(mut v) => { v.push(r); CxExpr::Sum(v) }
        ...
        loop {
            match self.peek().clone() {
                TokKind::Star => {
                    ...
                    factor = match factor {
                        CxExpr::Prod(mut v) => { v.push(r); CxExpr::Prod(v) }
    ...
    fn parse_cx_factor(&mut self) -> Result<CxExpr, ParseError> {
        match self.peek().clone() {
            TokKind::IntLit(1) => { self.bump(); Ok(CxExpr::Const) }
            ...
            TokKind::Ident(s) if s == "max" => {
                ...
                while matches!(self.peek(), TokKind::Comma) { self.bump(); xs.push(self.parse_cx_expr()?); }
                self.expect(&TokKind::RParen, "`)`")?;
                Ok(CxExpr::Max(xs))
            }
            TokKind::Ident(s) => {
    ...
    fn parse_sla_spec(&mut self) -> Result<Sla, ParseError> {
        ...
        loop {
            if !first {
                if !matches!(self.peek(), TokKind::Comma) { break; }
                self.bump();
            }
            first = false;
            let field = match self.peek().clone() {
                TokKind::Ident(s) => { self.bump(); s }
                TokKind::TypeName(s) => { self.bump(); s }
                _ => break,
            };
            let _le = matches!(self.peek(), TokKind::Le | TokKind::Eq);
            if matches!(self.peek(), TokKind::Le | TokKind::Eq) { self.bump(); }
            let r = self.parse_rat_or_duration()?;
            match field.as_str() {
                "e" => match r { Bound::Rat(p, q) => eps = (p, q), _ => return Err(self.err("e must be a rational".into())) },
                "d" => match r { Bound::Rat(p, q) => delta = (p, q), _ => return Err(self.err("d must be a rational".into())) },
                "T" => match r { Bound::Dur(ns) => t_ns = ns, _ => return Err(self.err("T must be a duration".into())) },
                other => return Err(self.err(format!("unknown sla field `{}`", other))),
            }
        }
        Ok(Sla { epsilon: eps, delta, t: Duration { ns: t_ns } })
    }

    fn parse_via_spec(&mut self) -> Result<Via, ParseError> {
        if matches!(self.peek(), TokKind::At) { self.bump(); }
        let tag = match self.peek().clone() {
            TokKind::Ident(s) => { self.bump(); Ident::new(s) }
        ...
        if !is_defined_via_tag(tag.as_str()) {
            return Err(self.err(format!(
                "via tag `@{}` has no defined semantics in APSL v0.1 — refusing to launder undefined obligation. \
                 Defined tags: @statistical. To add a tag, define its compositional obligation in docs/SPEC.yaml \
                 then add it to `is_defined_via_tag` in apsl-parse.",
                tag
            )));
        }
        ...
        loop {
            if matches!(self.peek(), TokKind::Comma) { self.bump(); }
            match self.peek().clone() {
                TokKind::Ident(s) => {
                    let save = self.pos;
                    self.bump();
        ...
        for required in required_attrs {
            if !attrs.iter().any(|(k, _)| k.as_str() == *required) {
                return Err(self.err(format!(
                    "via @{} requires attribute `{}=...`", tag, required
                )));
            }
        }
        Ok(Via { tag, attrs })
    ...
    fn parse_via_value(&mut self) -> Result<Ident, ParseError> {
        match self.peek().clone() {
            TokKind::Ident(s) | TokKind::TypeName(s) => { self.bump(); Ok(Ident::new(s)) }
            TokKind::IntLit(n) => { self.bump(); Ok(Ident::new(n.to_string())) }
            TokKind::StrLit(s) => { self.bump(); Ok(Ident::new(s)) }
        ...
        if let TokKind::Ident(u) = self.peek().clone() {
            if u == "ms" || u == "s" || u == "us" || u == "ns" {
                ...
                let mult = match u.as_str() {
                    "ns" => 1u128, "us" => 1_000, "ms" => 1_000_000, "s" => 1_000_000_000,
                    _ => unreachable!(),
                };
                if q == 0 { return Err(self.err("invalid rational".into())); }
                let ns = if p >= 0 { (p.unsigned_abs() * mult) / q } else { 0 };
    ...
    fn parse_number(&mut self) -> Result<(i128, u128), ParseError> {
        match self.peek().clone() {
            TokKind::IntLit(n) => { self.bump(); Ok((n, 1u128)) }
            TokKind::RatLit(p, q) => { self.bump(); Ok((p, q)) }
        ...
        loop {
            match self.peek().clone() {
                TokKind::Dot if self.allow_dot_postfix => {
                    ...
                    let field = match self.peek().clone() {
                        TokKind::Ident(s) => { self.bump(); Ident::new(s) }
                        TokKind::IntLit(n) => { self.bump(); Ident::new(n.to_string()) }
                        other => return Err(self.err(format!("expected field name after `.`, got {:?}", other))),
                    };
    ...
    fn parse_primary(&mut self) -> Result<Expr, ParseError> {
        let sp = self.span();
        match self.peek().clone() {
            TokKind::IntLit(n) => { self.bump(); Ok(Expr::Lit(Lit::Int(n), sp)) }
            TokKind::RatLit(p, q) => { self.bump(); Ok(Expr::Lit(Lit::Rat(p, q), sp)) }
            TokKind::BoolLit(b) => { self.bump(); Ok(Expr::Lit(Lit::Bool(b), sp)) }
            TokKind::StrLit(s) => { self.bump(); Ok(Expr::Lit(Lit::Str(s), sp)) }
            TokKind::LParen => {
                self.bump();
            ...
            TokKind::Ident(s) => {
                if s == "forall" || s == "exists" {
                    let q = if s == "forall" { Quant::Forall } else { Quant::Exists };
                    self.bump();
                    let xname = self.parse_ident()?;
                    match self.peek().clone() {
                        TokKind::Ident(k) if k == "in" => { self.bump(); }
                ...
                if s == "if" {
                    ...
                    match self.peek().clone() {
                        TokKind::Ident(k) if k == "then" => { self.bump(); }
                        _ => return Err(self.err("expected `then`".into())),
                    }
                    let a = self.parse_expr()?;
                    match self.peek().clone() {
                        TokKind::Ident(k) if k == "else" => { self.bump(); }
                ...
                if is_combinator(&s) {
                    ...
                    for _ in 0..arity {
                        if can_start_atom(self.peek()) { args.push(self.parse_atom_for_apply()?); }
                        else { break; }
                    }
                    return Ok(Expr::Apply(id, args, sp));
                }
            ...
            TokKind::TypeName(s) => { self.bump(); Ok(Expr::Var(Ident::new(s), sp)) }
...
fn reduce_rat(p: i128, q: u128) -> (i128, u128) {
    ...
    if g == 0 { (p, q) } else { (p / g as i128, q / g) }
}
...
fn is_combinator(s: &str) -> bool {
    matches!(s,
        "every" | "some" | "count" | "map" | "filter" | "fold"
        | "sort_by" | "group_by" | "sort"
        | "len" | "head" | "tail" | "nth" | "range"
        | "dedupe" | "concat" | "reverse" | "zip"
        | "unique?" | "subseteq?" | "valid_email?" | "well_formed_json?"
        | "not"
    ) || s.ends_with('?')
}
...
fn combinator_arity(s: &str) -> usize {
    match s {
        "every" | "some" | "count" | "map" | "filter" | "group_by" | "sort_by"
        | "subseteq?" | "nth" | "range" | "concat" | "zip" => 2,
        "fold" => 3,
        _ => 1,
    }
}
