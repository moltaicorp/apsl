...
fn via_service(n: &Node) -> Option<String> {
    n.via.as_ref().and_then(|v| {
        v.attrs
            ...
            .map(|(_, val)| val.as_str().to_string())
    })
}

fn emit_gitlab_child(
    prog: &apsl_core::Program,
    apsl_path: &str,
    hash: &str,
    ...
    for d in &prog.decls {
        match d {
            Decl::Node(n) => {
                nodes.insert(n.name.as_str().to_string(), n);
            }
            Decl::Graph(g) if graph.is_none() => {
                graph = Some(g);
            }
            _ => {}
        }
    ...
    for nm in &order {
        let n = nodes
            .get(nm)
            .ok_or_else(|| format!("flow references unknown node `{}`", nm))?;
        let dep = n
            .deploy
            .as_ref()
            .ok_or_else(|| format!("node `{}` has no --deploy clauses", nm))?;
        let stage = dep
            .stage
            .as_ref()
            .ok_or_else(|| format!("node `{}` has no `stage` clause", nm))?
            .as_str()
            .to_string();
    ...
    y.push('\n');
    y.push_str(&format!(
        "# GENERATED by `apslc deploy` from {}\n",
        apsl_path
    ));
    y.push_str(&format!(
        "# graph: {}   source hash: {}\n",
    ...
    for nm in &order {
        ...
        eprintln!(
            "[apslc deploy] job {} -> stage={} needs=[{}] extends=.{}_impl via={}",
            nm,
            stage,
            needs.join(","),
            nm,
    ...
    eprintln!(
        "[apslc deploy] {} jobs emitted; root proof {}",
        order.len(),
        type_to_nominal_name(&graph.sig.ret)
    );
    Ok(y)
}
